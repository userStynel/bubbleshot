class TerrainGenerator{
    ...
    getHeightIn(heights, xMin, zMin, xMax, zMax){
        ...
        for(var x = xMin; x < xMax; x++){
            for(var z = zMin; z < zMax; z++){
                ...
                let h = Maths.smoothInterpolation(bottomLeft, topLeft, bottomRight, topRight, xMin, xMax, zMin, zMax, x, z);
    ...
    getHeightMap(){
        ...
        var heights = [];
        for(let zPart = 0; zPart < part; zPart++){
            for(let xPart = 0; xPart < part; xPart++){
                this.getHeightIn(heights, xPart*PART_SIZE, zPart*PART_SIZE, (xPart+1)*PART_SIZE, (zPart+1)*PART_SIZE);
            }
        }
        return heights;
    }
